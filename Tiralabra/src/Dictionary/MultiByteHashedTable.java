...
public class MultiByteHashedTable {
     ...
     * @param refCount
     */
    public void purgeAndClean(int refCount) {
    ...
    /**
     * Builds a one-dimensional array of the contained multibyte entities, sorts the array 
     * descending by referencecount, ie the number of times a particular multibyte has been "called".
     * @return an array of multibytes sorted descending to reference count.
     */
    public MultiByte[] getArray() {
        ...
        MultiByte[] returnArray = extractMultiByteArray(objectArray);

        return returnArray;
    }
    
    /**
     * Extracts a one-dimensional MultiByte table from the two-dimensional Object
     * array given as parameter.
     * @param source
     * @return 
     */
    private MultiByte[] extractMultiByteArray(Object[][] source){
        MultiByte[] array = new MultiByte[source.length];
    ...
    /**
     * Merge-sorts the given array, where sorting is done descending according to the
     * reference count. This method requires a two-dimensional array where the first dimension
     * dictates the size of the array, and the subsequent subarrays 0 and 1 contain multibyte
     * data and its reference count respectively.
     * @param source
     * @return 
     */
    ...
    /**
     * Merges two arrays given as parameters 'firstPart' and 'secondPart' into one sorted array, also given as a parameter 'destination'.
     * @param firstPart
     * @param secondPart
     * @param destination 
     */
    ...
    /**
     * Clones objects from the 'source' array into the 'destination' array starting in the
     * source array at index 'from' and ending in 'until' and starting at 'destIndex' in the destination array.
     * The number of items cloned must never exceed the capacity of the destination array, ie
     * the final value of destIndex must always end no later than the capacity of destination -1.
     * @param source
     * @param destination
     * @param from
     * @param until
