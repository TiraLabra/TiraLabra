...
public class Pelilauta {
    ...
    /**
     * Metodi palauttaa listan mahdollisista siirroista. Jos vuorossa olevalla pelaajalla on mahdollisia hyppyjä, ne lisätään ensin listaan.
     * Jos hyppyjä ei ole, lisätään listaan muut siirrot.
     *
    ...
    public Siirto[] getSallitutSiirrot(boolean valkoisenVuoroSiirtaa) {
        Lista<Siirto> mahdollisetSiirrot = new Lista<>();
        ...
        if (!mahdollisetSiirrot.isEmpty()) {
            Siirto[] siirrot = new Siirto[mahdollisetSiirrot.getKoko()];
            for (int i = 0; i < mahdollisetSiirrot.getKoko(); i++) {
                siirrot[i] = mahdollisetSiirrot.getAlkio(i);
            }
            return siirrot;
        }
        return null;
    ...
    private void lisaaMahdollisetSiirrotListaan(boolean valkoisenVuoroSiirtaa, Lista<Siirto> mahdollisetSiirrot) {
        for (int rivi = 0; rivi < 8; rivi++) {
            for (int sarake = 0; sarake < 8; sarake++) {
                ...
                if (voikoNappulaLiikkua(valkoisenVuoroSiirtaa, rivi, sarake, rivi + 1, sarake + 1)) {
                    mahdollisetSiirrot.lisaa(new Siirto(rivi, sarake, rivi + 1, sarake + 1));
                }
                if (voikoNappulaLiikkua(valkoisenVuoroSiirtaa, rivi, sarake, rivi - 1, sarake + 1)) {
                    mahdollisetSiirrot.lisaa(new Siirto(rivi, sarake, rivi - 1, sarake + 1));
                }
    ...
    private void lisaaMahdollisetHypytListaan(boolean valkoisenVuoroSiirtaa, Lista<Siirto> mahdollisetSiirrot) {
        for (int rivi = 0; rivi < 8; rivi++) {
            for (int sarake = 0; sarake < 8; sarake++) {
                if (ruudukko[rivi][sarake] == null) {
                    continue;
                ...
                if (voikoNappulaHypata(valkoisenVuoroSiirtaa, rivi, sarake, rivi + 1, sarake + 1, rivi + 2, sarake + 2)) {
                    mahdollisetSiirrot.lisaa(new Siirto(rivi, sarake, rivi + 2, sarake + 2));
                }
                if (voikoNappulaHypata(valkoisenVuoroSiirtaa, rivi, sarake, rivi - 1, sarake + 1, rivi - 2, sarake + 2)) {
                    mahdollisetSiirrot.lisaa(new Siirto(rivi, sarake, rivi - 2, sarake + 2));
                }
     ...
     * ruudusta toiseen ja tarvittaessa poistaa syödyn nappulan pelilaudalta
     *
