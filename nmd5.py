...
class NMD5:
	...
	def hash(self, message):
		...
		for chunk in chunks:
			...
			words = self.createWordArray(chunk, messageLength)
			...
			self.buffers[0] = (a + A) % pow(2,32)
			self.buffers[1] = (b + B) % pow(2,32)
			self.buffers[2] = (c + C) % pow(2,32)
			self.buffers[3] = (d + D) % pow(2,32)
	...
	def createWordArray(self, message, messageLength):
