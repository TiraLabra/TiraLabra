...
Ohjelmassa "pääpaketti" com.mycompany.Tiralabra_maven, jossa on ajettava tiedosto App.java.

Paketissa com.mycompany.Tiralabra_maven.logiikka on 2 luokka. Luokka Paikka kuvaa yhtä paikkaa kartalla, jossa liikutaan. Tätä luokkaa käytetään kun algoritmeissa etsitään nopeinta reittiä. Luokka Piste on vain lähtö- ja maalipisteiden koordinaattien antoa varten.
...
Minimikeko on toteutettu 1-ulotteisena taulukkona, johon on talletetut kekoalkiot ovat Paikka-olioita. Huipulla keossa olevan alkion indeksi on 1. Kekoalkioiden avainten vertailut on toteutettu Paikka-luokan metodia compareTo käyttäen, jotta omaa minimikekoa ja Javan Priorityqueue:ta hyödyntävää kekoa voidaan käyttää samalla tavalla ohjelmaa muuttamatta kun siirrytään Javan toteutuksesta minimikeon omaan toteutukseen.
